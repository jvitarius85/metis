...
class JobQueue {
    ...
    public function recoverExpiredProcessingJobs(): array {
        ...
        $failed = $this->database()->execute( (string) $this->database()->prepare(
            "UPDATE {$table}
             ...
             WHERE status = %s
               ...
               AND reserved_until <> ''
               AND (reserved_until < %s OR reserved_until > %s)
               AND attempts >= max_attempts",
            self::STATUS_FAILED,
            $now,
            'Job lease expired after maximum attempts. Review logs before retrying.',
            self::STATUS_PROCESSING,
            $now,
            $futureSkewCutoff
        ) );

        $requeued = $this->database()->execute( (string) $this->database()->prepare(
            "UPDATE {$table}
             SET status = %s,
                 reserved_at = NULL,
                 reserved_until = NULL,
             ...
             WHERE status = %s
               ...
               AND attempts < max_attempts",
            self::STATUS_QUEUED,
            'Job lease expired and was returned to the queue.',
            self::STATUS_PROCESSING,
            $now,
            $futureSkewCutoff
        ) );
        ...
        return [
            ...
            'requeued' => max( 0, (int) $requeued ),
        ];
    }

    public function pruneCompletedJobs( int $retentionDays = 14, int $limit = 5000 ): int {
        $retentionDays = max( 1, min( 365, $retentionDays ) );
        $limit = max( 100, min( 20000, $limit ) );
        $table = \Metis_Tables::get( 'job_queue' );
        $cutoff = $this->formatTimestamp( $this->currentLocalTimestamp() - ( $retentionDays * 86400 ) );
    ...
    private function markFailed( array $job, \Throwable $e ): string {
        ...
        if ( $attempts < $max_attempts ) {
            $retry_at = $this->formatTimestamp( $this->currentLocalTimestamp() + min( 3600, max( 60, $attempts * 60 ) ) );
            $this->database()->update(
                $table,
                [
                    'status'         => self::STATUS_QUEUED,
    ...
    private function currentLocalTimestamp(): int {
        $timestamp = strtotime( \metis_current_time( 'mysql' ) );
        return $timestamp !== false ? (int) $timestamp : time();
    }
