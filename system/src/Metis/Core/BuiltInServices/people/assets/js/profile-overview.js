...
window.MetisPeopleProfileModules.initOverview = function (context) {
            ...
            addPeopleForm.addEventListener('submit', function (event) {
                ...
                post('metis_people_save_person', {
                    ...
                    if (pid && rowsWrap) {
                        ...
                        row.innerHTML =
                            '<div class="metis-premium-cell"><strong>' + personName + '</strong><div class="metis-muted">' + pid + '</div></div>' +
                            '<div class="metis-premium-cell">' + personEmail + '</div>' +
                            '<div class="metis-premium-cell">Metis</div>' +
                            '<div class="metis-premium-cell">—</div>' +
                            '<div class="metis-premium-cell">No</div>' +
                            '<div class="metis-premium-cell">—</div>' +
                            '<div class="metis-premium-cell">' + (href ? ('<a href="' + href + '" class="metis-btn-xs">Edit</a>') : '—') + '</div>';
            ...
            addRoleForm.addEventListener('submit', function (event) {
                ...
                post('metis_people_save_role', {
                    ...
                    if (savedRole && roleRowsWrap) {
                        ...
                        if (href) row.dataset.href = href;
                        row.innerHTML =
                            '<div class="metis-premium-cell metis-role-col-key">' + savedRole + '</div>' +
                            '<div class="metis-premium-cell metis-role-col-name">' + roleNameText + '</div>' +
                            '<div class="metis-premium-cell metis-role-col-perms">0</div>' +
                            '<div class="metis-premium-cell metis-role-col-members">0</div>' +
                            '<div class="metis-premium-cell metis-role-col-system">No</div>' +
                            '<div class="metis-premium-cell metis-role-col-actions">' + (href ? ('<a href="' + href + '" class="metis-btn-xs">Edit</a>') : '—') + '</div>';
    ...
    if (positionsPage) {
        ...
        function renderPositionList(groupKey, options) {
            ...
            if (!list.length) {
                wrap.innerHTML = '<div class="metis-muted">No positions configured.</div>';
                return;
            }
    ...
    if (canManage && docForm) {
        docForm.addEventListener('submit', function (event) {
            ...
            post('metis_people_add_document', {
                ...
                if (!list || !row) return;
                const empty = list.querySelector('.metis-muted');
                ...
                item.innerHTML =
                    ...
                    '</div>' +
                    '<div class="metis-people-mini-actions"><button type="button" class="metis-btn-xs metis-btn-danger metis-doc-delete">Delete</button></div>';
        ...
        btn.addEventListener('click', function () {
            ...
            post('metis_people_complete_lifecycle_task', { task_id: taskId })
                .then(function () {
                    ...
                    const row = btn.closest('.metis-people-mini-item');
                    if (row) {
                        const meta = row.querySelector('.metis-muted');
