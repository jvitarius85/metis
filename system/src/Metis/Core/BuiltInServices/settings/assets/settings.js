function metisInitSettingsUi() {
    ...
    function hideAboutProgress(key) {
        const panel = aboutProgressPanel(key);
        if (!panel) return;
        panel.hidden = true;
    ...
    function bindSettingsAsyncActions(scope) {
        ...
        (scope || document).querySelectorAll('[data-release-apply-tag]').forEach(function (button) {
            if (button.dataset.metisBound === '1') return;
            button.dataset.metisBound = '1';
            button.addEventListener('click', function () {
    ...
    function releaseProgressPanel(button, tag) {
        updateAboutProgress('core', {
            title: 'Release Update ' + tag,
            percent: 0,
            stage: 'running',
            message: 'Preparing update...'
        });
        return aboutProgressPanel('core');
    }

    function updateReleaseProgressPanel(panel, progress) {
        if (!panel) return;
        const key = panel.getAttribute('data-about-progress-panel') === 'modules' ? 'modules' : 'core';
        updateAboutProgress(key, progress);
    }
