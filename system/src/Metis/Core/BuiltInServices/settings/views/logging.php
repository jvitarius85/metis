...
$logging_clear_action_nonce = function_exists( 'metis_runtime_create_nonce' ) && function_exists( 'metis_ajax_nonce_action' )
    ...
    : '';
$settings_js_string = static function ( mixed $value ): string {
    $encoded = metis_json_encode( (string) $value, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT );
...
<p class="metis-subtitle">Set log level and review events.</p>
<?php metis_settings_render_messages( $saved, $errors ); ?>
<?php metis_settings_render_section_nav( 'logging' ); ?>
<form method="post" class="metis-settings-form" data-metis-settings-form="1" data-settings-section="logging">
    ...
    <div class="metis-settings-card">
        <div class="metis-settings-header"><h2>Logging Controls</h2></div>
        <div class="metis-settings-body">
            <div class="metis-field">
                ...
                <p class="metis-help">When disabled, Metis logger output is suppressed for all levels.</p>
            </div>
            <div class="metis-field">
            ...
            <div class="metis-field">
                ...
                <div class="metis-help"><code><?php echo metis_escape_html( (string) $logging_log_path ); ?></code></div>
            </div>
        </div>
    </div>
    <div class="metis-settings-actions">
        <button type="submit" class="metis-btn">Save Logging Settings</button>
    </div>
</form>
<div class="metis-settings-card" id="metis-live-log-viewer">
    <div class="metis-settings-header"><h2>Live Log Viewer</h2></div>
    <div class="metis-settings-body">
            <div class="metis-field" style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
                <button type="button" id="logging_refresh_btn" class="metis-btn metis-btn-ghost">Refresh</button>
                <button type="button" id="logging_clear_btn" class="metis-btn metis-btn-ghost">Clear Log</button>
                ...
                <select id="logging_auto_refresh_seconds" name="logging_auto_refresh_seconds" class="metis-input" style="min-width:84px;">
            ...
            <details class="metis-field" style="margin-bottom:8px;">
                ...
                <div style="display:flex;flex-direction:row;gap:10px;align-items:center;flex-wrap:nowrap;overflow-x:auto;white-space:nowrap;margin-top:8px;padding-bottom:2px;">
                    ...
                    <select id="logging_view_file" name="logging_view_file" class="metis-input" style="min-width:250px;">
                    ...
                    <select id="logging_view_lines" name="logging_view_lines" class="metis-input" style="min-width:130px;width:130px;">
                    ...
                    <input type="text" id="logging_search" name="logging_search" class="metis-input" style="min-width:240px;" value="<?php echo metis_escape_attr( (string) ( $logging_search ?? '' ) ); ?>" placeholder="message, level, context...">
                </div>
            </details>
            <div class="metis-help" id="logging_entries_loaded" style="margin-bottom:10px;">Loaded <?php echo metis_escape_html( number_format( (int) ( $logging_total_entries ?? count( (array) $logging_entries ) ) ) ); ?> entries.</div>
            <div class="metis-help" id="logging_viewer_error" style="display:none;margin-bottom:10px;color:#b42318;"></div>
            <div style="border:1px solid #d0d7de;border-radius:12px;overflow:auto;max-height:620px;background:#fff;">
                <div class="metis-help" style="padding:8px 12px;border-bottom:1px solid #eef2f7;background:#f8fafc;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;margin-top:10px;gap:10px;flex-wrap:wrap;">
                ...
                <div style="display:flex;gap:8px;align-items:center;">
                    <button type="button" id="logging_prev_btn" class="metis-btn metis-btn-ghost" <?php echo (int) ( $logging_page ?? 1 ) <= 1 ? 'disabled' : ''; ?>>Previous</button>
                    <button type="button" id="logging_next_btn" class="metis-btn metis-btn-ghost" <?php echo (int) ( $logging_page ?? 1 ) >= (int) ( $logging_total_pages ?? 1 ) ? 'disabled' : ''; ?>>Next</button>
                    <label for="logging_page_jump" style="font-weight:600;margin:0;">Go to</label>
                    <input type="number" id="logging_page_jump" name="logging_page_jump" min="1" max="<?php echo metis_escape_attr( (string) max( 1, (int) ( $logging_total_pages ?? 1 ) ) ); ?>" value="<?php echo metis_escape_attr( (string) ( $logging_page ?? 1 ) ); ?>" class="metis-input" style="width:90px;">
                    <button type="button" id="logging_jump_btn" class="metis-btn metis-btn-ghost">Go</button>
                </div>
            </div>
        </div>
    </div>
...
(function() {
    ...
    var currentPage = parseInt(<?php echo $settings_js_string( (string) ( $logging_page ?? 1 ) ); ?>, 10) || 1;
    var totalPages = parseInt((els.jumpInput && els.jumpInput.max) || '1', 10) || 1;
    var searchTimer = null;
    var liveTimer = null;
    var loading = false;
    var serverAjaxUrl = <?php echo $settings_js_string( (string) metis_ajax_endpoint_url() ); ?>;
    var serverNonce = <?php echo $settings_js_string( (string) metis_runtime_create_nonce( 'metis_core' ) ); ?>;
    var viewerAjax = (function() {
        ...
        return {
            ...
            action_nonces: Object.assign({}, globalAjax.action_nonces || {}, coreAjax.action_nonces || {}, {
                metis_settings_fetch_logging_viewer: <?php echo $settings_js_string( $logging_fetch_action_nonce ); ?>,
                metis_settings_clear_log: <?php echo $settings_js_string( $logging_clear_action_nonce ); ?>
            })
        };
    })();
