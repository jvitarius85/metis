...
if ( ! function_exists( 'metis_settings_build_stripe_diagnostics_snapshot' ) ) {
    function metis_settings_build_stripe_diagnostics_snapshot( string $date_format, string $time_format, string $timezone ): array {
        ...
        $telemetry = \Metis\Modules\Settings\SettingsTelemetryService::stripeWebhookSnapshot( 'stripe', 1 );
        $snapshot['webhook_events_24h'] = (int) ( $telemetry['webhook_events'] ?? 0 );
        $snapshot['webhook_processed_24h'] = (int) ( $telemetry['webhook_processed'] ?? 0 );
        $snapshot['webhook_failed_24h'] = (int) ( $telemetry['webhook_failed'] ?? 0 );
        $snapshot['provider_failure_count'] = (int) ( $telemetry['provider_failure_count'] ?? 0 );
...
if ( ! function_exists( 'metis_settings_health_code_lookup_status' ) ) {
    function metis_settings_health_code_lookup_status(): array {
        ...
        return \Metis\Modules\Settings\SettingsTelemetryService::codeLookupStatus();
    }
}
...
if ( ! function_exists( 'metis_settings_build_performance_security_report' ) ) {
    function metis_settings_build_performance_security_report(): array {
        ...
        $security_offense_summary = \Metis\Modules\Settings\SecurityOffenseService::summarizeLastDays( 7 );
        $security_offense_total = (int) ( $security_offense_summary['total'] ?? 0 );
        $security_offense_top = (string) ( $security_offense_summary['top'] ?? '' );
        $security_offense_breakdown = array_values( array_filter( array_map( 'strval', (array) ( $security_offense_summary['breakdown'] ?? [] ) ) ) );
        $security_cutoff = (string) ( $security_offense_summary['cutoff'] ?? metis_settings_recent_cutoff( 7 ) );
        ...
        $security_pressure = \Metis\Modules\Settings\SettingsTelemetryService::securityPressureSummary( $security_cutoff );
        $brute_force_total = (int) ( $security_pressure['brute_force_total'] ?? 0 );
        $rate_limit_total = (int) ( $security_pressure['rate_limit_total'] ?? 0 );
        $brute_force_top = array_values( array_filter( array_map( 'strval', (array) ( $security_pressure['brute_force_top'] ?? [] ) ) ) );
        $rate_limit_top = array_values( array_filter( array_map( 'strval', (array) ( $security_pressure['rate_limit_top'] ?? [] ) ) ) );
        ...
        $webhook_summary = \Metis\Modules\Settings\SettingsTelemetryService::webhookHealthSummary( 7 );
        $webhook_processed_7d = (int) ( $webhook_summary['processed'] ?? 0 );
        $webhook_failed_7d = (int) ( $webhook_summary['failed'] ?? 0 );
        $webhook_last_processed_at = (string) ( $webhook_summary['last_processed_at'] ?? '' );
...
if ( ! function_exists( 'metis_settings_build_failed_login_snapshot' ) ) {
    function metis_settings_build_failed_login_snapshot( string $date_format, string $time_format, string $timezone, int $limit = 25 ): array {
        return \Metis\Modules\Settings\SettingsTelemetryService::failedLoginSnapshot( $date_format, $time_format, $timezone, $limit );
    }
}
