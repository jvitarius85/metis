...
metis_ajax_register_handler( 'metis_resolve_code', function () {
    ...
    if ( ! class_exists( '\Metis\Core\Services\AjaxCodeLookupService' ) ) {
        metis_runtime_send_json_error( [ 'message' => 'Code lookup service is unavailable.' ], 500 );
    }

    metis_runtime_send_json_success(
        \Metis\Core\Services\AjaxCodeLookupService::resolveCodePayload( $code, $fuzzy )
    );
} );
