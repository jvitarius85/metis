...
$finance = $read( 'modules/Finance/FinanceV2Service.php' );
...
$ajaxCodeLookupService = $read( 'src/Metis/Core/Services/AjaxCodeLookupService.php' );
$codeRegistry = $read( 'src/Metis/Core/CodeRegistry.php' );
$loggerRuntime = $read( 'src/Metis/Core/LoggerRuntime.php' );
$boardSupport = $read( 'modules/Board/Support.php' );
$contactsSupport = $read( 'modules/Contacts/Support.php' );
$newsletterSupport = $read( 'modules/Newsletter/Support.php' );
...
$websiteThemeService = $read( 'modules/Website/Services/ThemeService.php' );
$websiteRenderer = $read( 'modules/Website/Services/WebsiteRenderer.php' );
$websiteAjax = $read( 'modules/website/ajax/website.ajax.php' );
$websiteThemeView = $read( 'modules/website/views/theme.php' );
$websiteThemeAdminCss = $read( 'modules/website/assets/theme-admin.css' );
...
$assert( str_contains( $codeRegistry, 'function search' ) && str_contains( $codeRegistry, 'searchable_columns' ) && str_contains( $codeRegistry, 'normalize_search_query' ), 'Code registry must expose bounded registry-backed keyword search over safe entity label fields.' );
...
$assert( str_contains( $ajaxRuntime, 'metis_rehydrate_code_lookup' ) && str_contains( $ajaxCodeLookupService, "'found' => false" ), 'Core AJAX must expose governed code lookup rehydration and return 200-level lookup misses.' );
$assert( str_contains( $ajaxRuntime, 'AjaxCodeLookupService::resolveCodePayload' ), 'Core AJAX code lookup must delegate to the canonical lookup service.' );
$assert( str_contains( $ajaxCodeLookupService, 'Metis_Code_Registry::search' ) && str_contains( $ajaxCodeLookupService, '$max_results = 25' ), 'Canonical AJAX code lookup service must include bounded keyword matches without creating a second lookup endpoint.' );
$assert( ! str_contains( $ajaxRuntime, "Code not found.' ], 404" ), 'Code lookup misses must not surface as AJAX 404 failures.' );
$assert( str_contains( $coreJs, "r.success && (!r.data || r.data.found !== false)" ) && str_contains( $coreJs, "metis_csrf_action" ) && str_contains( $coreJs, 'MIN_CODE_LENGTH = 3' ), 'Core code search must handle lookup misses, send explicit CSRF action metadata, and support three-character keyword searches.' );
$assert( str_contains( $websiteThemeView, 'metis-theme-menu-style' ) && str_contains( $websiteThemeView, "'h_glide' => 'Glide Gradient'" ) && str_contains( $websiteThemeView, "'h_pill_dropdown' => 'Pill Dropdown'" ) && str_contains( $websiteThemeView, "'h_modern_bar' => 'Modern Bar'" ) && ! str_contains( $websiteThemeView, "'h_marker_dropdown' => 'Marker Dropdown'" ) && ! str_contains( $websiteThemeView, "'h_solid_bar' => 'Solid Bar'" ) && ! str_contains( $websiteThemeView, "'h_clean' => 'Clean'" ) && str_contains( $websiteThemeView, 'menuPresetDefinitions' ) && ! str_contains( $websiteThemeView, 'metis-theme-menu-font-size' ) && ! str_contains( $websiteThemeView, 'metis-theme-menu-color' ) && ! str_contains( $websiteThemeView, 'menuStyleFromLayout' ), 'Website theme editor must expose only governed public menu presets without custom desktop/dropdown/mobile controls or layout fallback mapping.' );
$assert( str_contains( $websiteThemeService, 'menuPresetConfig' ) && str_contains( $websiteThemeService, "'h_glide'" ) && str_contains( $websiteThemeService, "'font_size' => 19" ) && str_contains( $websiteThemeService, '$menu_mobile_breakpoint = 980' ) && ! str_contains( $websiteThemeService, 'legacyMenuStyleFromLayout' ) && ! str_contains( $websiteRenderer, 'metis-menu-mobile-overlay' ) && ! str_contains( $websiteRenderer, 'data-metis-nav-mobile-type' ) && str_contains( $websiteRenderer, 'metis-menu-style-h_glide' ) && str_contains( $websiteRenderer, 'codepenGlideMenuCss' ) && str_contains( $websiteRenderer, ':last-child::after' ) && str_contains( $websiteRenderer, '--metis-glide-primary:var(--metis-color-primary' ) && str_contains( $websiteRenderer, '--metis-glide-accent:var(--metis-color-accent' ) && str_contains( $websiteRenderer, '--metis-glide-menu-width:62rem' ) && str_contains( $websiteRenderer, 'flex:0 1 var(--metis-glide-menu-width)' ) && str_contains( $websiteRenderer, 'gap:48px !important' ) && str_contains( $websiteRenderer, 'max-width:1320px' ) && str_contains( $websiteRenderer, 'flex:1 1 0 !important' ) && str_contains( $websiteRenderer, 'width:min(100%,var(--metis-glide-menu-width))' ) && str_contains( $websiteRenderer, 'text-overflow:ellipsis' ) && str_contains( $websiteRenderer, 'stickyHeaderCss' ) && str_contains( $websiteRenderer, 'position:fixed !important;top:0;left:0;right:0;width:100%' ) && str_contains( $websiteRenderer, 'padding-top:var(--metis-fixed-header-space' ) && str_contains( $websiteRenderer, 'is-scrolled' ) && str_contains( $websiteRenderer, 'white-space:nowrap' ) && str_contains( $websiteRenderer, 'gap:0 !important' ) && str_contains( $websiteRenderer, 'cubic-bezier(1,.49,.09,1.29)' ) && str_contains( $websiteRenderer, '150 * $position' ) && str_contains( $websiteRenderer, 'isHomepageComparablePath' ) && str_contains( $websiteThemeView, 'metis-theme-menu-preview-row' ) && str_contains( $websiteThemeView, 'renderThemeMenuPreviewCss' ) && str_contains( $websiteThemeView, 'font_size: 19' ) && str_contains( $websiteThemeView, "removeClass('is-active is-active-ancestor')" ) && str_contains( $websiteThemeAdminCss, '.metis-theme-menu-live.metis-menu-style-h_glide' ) && str_contains( $websiteThemeAdminCss, '--metis-glide-menu-width:62rem' ) && ! str_contains( $websiteRenderer, '#e91e63' ) && ! str_contains( $websiteThemeAdminCss, '#e91e63' ) && ! str_contains( $websitePublicNavigation, 'setupGlideMenus' ) && ! str_contains( $websitePublicNavigation, 'item.offsetWidth' ), 'Public website renderer must render Glide Gradient as a governed preset with exact theme colors, bounded flexible desktop slot sizing, rendered theme preview parity, fixed sticky shrink behavior, homepage-active suppression, and no JavaScript geometry writes.' );
$assert( ! str_contains( $websiteThemeView, "'h_marker_dropdown' => 'Marker Dropdown'" ) && ! str_contains( $websiteThemeService, "'h_marker_dropdown'" ) && ! str_contains( $websiteRenderer, 'markerDropdownMenuCss' ) && ! str_contains( $websiteRenderer, 'metis-menu-style-h_marker_dropdown' ) && ! str_contains( $websiteRenderer, '--metis-marker-count' ) && ! str_contains( $websiteRenderer, '--metis-marker-bg:var(--metis-color-primary' ) && ! str_contains( $websiteRenderer, '--metis-marker-accent:var(--metis-color-accent' ) && ! str_contains( $websiteRenderer, 'min-width:500px' ) && ! str_contains( $websiteRenderer, 'translate(0,-100%)' ) && ! str_contains( $websiteRenderer, '--metis-marker-index' ) && ! str_contains( $websiteThemeAdminCss, '.metis-theme-menu-live.metis-menu-style-h_marker_dropdown' ) && ! str_contains( $websiteThemeAdminCss, '--metis-marker-bg:var(--metis-color-primary' ) && ! str_contains( $websitePublicNavigation, '--metis-marker-left' ), 'Marker Dropdown must be fully removed from website theme presets, renderer CSS, and preview styling.' );
$assert( str_contains( $websiteTemplateHomepage, 'data-metis-nav-toggle' ) && str_contains( $websiteTemplatePage, 'data-metis-nav-toggle' ) && str_contains( $websiteTemplatePost, 'data-metis-nav-toggle' ) && str_contains( $websiteTemplateHomepage, 'metis-shell-nav-toggle-lines' ) && str_contains( $websiteTemplatePage, 'metis-shell-nav-primary' ) && str_contains( $websiteRenderer, 'mobileFlyoutMenuCss' ) && str_contains( $websiteRenderer, 'translateX(105%)' ) && str_contains( $websiteRenderer, 'metis-nav-open' ) && str_contains( $websitePublicNavigation, 'aria-expanded' ), 'Default website templates must render the governed mobile flyout trigger and keep the hamburger/X state synchronized through the public navigation runtime.' );
$assert( str_contains( $websiteThemeService, "'h_pill_dropdown'" ) && str_contains( $websiteThemeService, "'h_modern_bar'" ) && str_contains( $websiteRenderer, 'pillDropdownMenuCss' ) && str_contains( $websiteRenderer, 'modernBarMenuCss' ) && str_contains( $websiteRenderer, 'metis-menu-style-h_pill_dropdown' ) && str_contains( $websiteRenderer, 'metis-menu-style-h_modern_bar' ) && str_contains( $websiteRenderer, 'border-radius:999px' ) && str_contains( $websiteRenderer, 'scale(.96)' ) && str_contains( $websiteRenderer, 'scaleX(0)' ) && str_contains( $websiteThemeAdminCss, '.metis-theme-menu-live.metis-menu-style-h_pill_dropdown' ) && str_contains( $websiteThemeAdminCss, '.metis-theme-menu-live.metis-menu-style-h_modern_bar' ), 'Public website renderer must expose Pill Dropdown and Modern Bar as governed CSS presets with animated dropdown behavior.' );
$assert( str_contains( $websiteThemeView, 'theme_save_mode' ) && str_contains( $websiteThemeView, 'theme_patch_json' ) && str_contains( $websiteThemeView, 'markThemeDirtyPath' ) && str_contains( $websiteThemeView, 'normalizeThemeForEditor' ), 'Website theme editor must save changed theme sections as scoped patches instead of blind full-payload overwrites.' );
$assert( str_contains( $websiteAjax, 'metis_website_ajax_theme_patch_allowlist' ) && str_contains( $websiteAjax, 'theme_patch_paths' ) && str_contains( $websiteAjax, 'metis_website_ajax_theme_apply_patch' ), 'Website theme AJAX must apply only allowlisted scoped theme patch paths.' );
$assert( str_contains( $websiteThemeService, '$custom_tokens_root' ) && str_contains( $websiteThemeView, "\$custom_input = [ 'tokens' => \$custom_input ]" ), 'Website theme custom token persistence must preserve the editor token envelope.' );
$assert( str_contains( $websiteThemeService, "'layout' => \$normalized['global_styles']['layout']" ) && str_contains( $websiteThemeView, "'layout'          => is_array( \$theme['layout']" ) && str_contains( $websiteThemeView, 'layout: src.layout || {}' ), 'Website theme layout settings must round-trip through normalized saves and AJAX responses.' );
...
$assert( str_contains( $settingsRuntime, 'metis-failed-logins-table' ) && str_contains( $settingsCss, '.metis-failed-logins-table .metis-premium-row' ) && str_contains( $settingsCss, 'grid-template-columns: minmax(140px' ), 'Failed Logins panel must render as a real multi-column Metis table.' );
