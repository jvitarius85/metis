(function (window, document) {
    ...
    if (initialOptions && typeof initialOptions === 'object') {
        state.options = {
            ...
            newsletterLists: Array.isArray(initialOptions.newsletter_lists) ? initialOptions.newsletter_lists : [],
    ...
    function siteBaseUrl() {
        var website = editorModuleAjaxConfig();
        var core = window.metisAjax || {};
        var raw = s(website.site_url || core.site_url || '').trim();
    ...
    function relativePublicUrl(value) {
        var raw = s(value || '').trim();
        if (!raw) return '';
        if (raw.charAt(0) === '/') return raw;
        var siteUrl = siteBaseUrl();
        if (siteUrl && raw.indexOf(siteUrl + '/') === 0) {
            return raw.slice(siteUrl.length);
        }
        try {
            var parsed = new URL(raw, window.location.href);
            var site = siteUrl ? new URL(siteUrl + '/', window.location.href) : null;
            if (site && parsed.origin === site.origin) {
                return s(parsed.pathname || '/') + s(parsed.search || '') + s(parsed.hash || '');
            }
        } catch (_err) {}
        return raw;
    }

    function mediaPublicUrlForRow(row) {
        var data = row && typeof row === 'object' ? row : {};
        var token = s(data.value || data.token || data.public_token || '').trim();
        if (token) return '/media/' + encodeURIComponent(token);
        return relativePublicUrl(s(data.url || data.src || ''));
    }

    var FEATURE_GRID_ICON_CATEGORY_LABELS = {
        all: 'All',
    ...
    var FEATURE_GRID_ICON_LIBRARY = (function () {
        var iconLibraryEl = document.getElementById('metis-editor-icon-library-json') || document.getElementById('metis-menu-icon-library-json');
        if (iconLibraryEl) {
            try {
                var parsed = JSON.parse(String(iconLibraryEl.textContent || '[]'));
                if (Array.isArray(parsed) && parsed.length) {
                    return parsed.filter(function (entry) {
                        if (!entry || typeof entry.key !== 'string') return false;
                        var hasSvg = typeof entry.svg === 'string' && entry.svg.trim() !== '';
                        var hasUrl = typeof entry.url === 'string' && entry.url.trim() !== '';
                        return hasSvg || hasUrl;
                    }).map(function (entry) {
                        var normalized = Object.assign({}, entry);
                        normalized.key = s(normalized.key || '').replace(/^icon:/, '').replace(/_/g, '-');
                        normalized.label = s(normalized.label || normalized.key);
                        normalized.category = s(normalized.category || 'general').toLowerCase() || 'general';
                        normalized._search = (normalized.label + ' ' + normalized.key + ' ' + normalized.category).toLowerCase();
                        return normalized;
                    });
                }
            } catch (_err) {}
        ...
        return [
            'accessibility', 'calendar', 'chat', 'contacts', 'dashboard', 'database',
            'disability-ramp', 'globe', 'heart', 'home', 'image', 'location',
            'megaphone', 'people', 'phone', 'resources', 'star', 'support', 'website'
        ].map(function (slug) {
            var key = s(slug || '').replace(/_/g, '-');
            var label = key.split('-').map(function (part) {
            ...
            return {
                ...
                label: label,
                category: 'general',
                _search: (label + ' ' + key + ' general').toLowerCase()
            };
        });
    })();
    ...
    function featureGridIconEntry(key) {
        var lookup = s(key || '').trim().replace(/_/g, '-');
        if (!lookup) return null;
        for (var i = 0; i < FEATURE_GRID_ICON_LIBRARY.length; i += 1) {
            if (FEATURE_GRID_ICON_LIBRARY[i].key === lookup) return FEATURE_GRID_ICON_LIBRARY[i];
        }
        return null;
    }

    function featureGridIconMarkup(key) {
        var entry = featureGridIconEntry(key);
        if (entry && typeof entry.svg === 'string' && entry.svg.trim()) {
            return entry.svg;
        }
        if (entry && typeof entry.url === 'string' && entry.url.trim()) {
            return '<img src="' + esc(entry.url) + '" alt="' + esc(entry.label || entry.key || 'Icon') + '" loading="lazy" decoding="async">';
        }
        var slug = s(key || '').trim().replace(/_/g, '-');
        if (!slug) return '';
    ...
    function featureGridCloseIconPanel() {
    ...
    function featureGridApplyIconFieldState(scope) {
        var rootNode = scope && scope.querySelectorAll ? scope : document;
        rootNode.querySelectorAll('[data-v2-feature-grid-icon-field]').forEach(function (fieldRoot) {
            var input = fieldRoot.querySelector('[data-v2-feature-grid-icon-input]');
            var preview = fieldRoot.querySelector('[data-v2-feature-grid-icon-preview]');
            var token = fieldRoot.querySelector('[data-v2-feature-grid-icon-token]');
            var raw = s(input && input.value || '').trim().replace(/_/g, '-');
            var entry = featureGridIconEntry(raw);
            if (preview) {
                preview.innerHTML = raw ? featureGridIconMarkup(raw) : '';
            }
            if (token) {
                token.textContent = entry ? entry.label : (raw ? raw : 'No icon selected');
            }
        });
        bindIconFallbacks(rootNode);
    }
    ...
    function featureGridOpenIconPanel(fieldRoot, anchorButton) {
        ...
        function renderTabs() {
            tabs.innerHTML = '';
            var categories = ['all'];
            FEATURE_GRID_ICON_LIBRARY.forEach(function (entry) {
                var category = s(entry.category || 'general').toLowerCase() || 'general';
                if (categories.indexOf(category) === -1) categories.push(category);
            });
            categories = ['all'].concat(categories.slice(1).sort(function (a, b) {
                return s(FEATURE_GRID_ICON_CATEGORY_LABELS[a] || a).localeCompare(s(FEATURE_GRID_ICON_CATEGORY_LABELS[b] || b));
            }));
            categories.forEach(function (category) {
                var tab = document.createElement('button');
                tab.type = 'button';
                tab.className = 'metis-menu-icon-tab' + (category === activeCategory ? ' is-active' : '');
                tab.setAttribute('data-v2-feature-grid-icon-tab', category);
                tab.textContent = FEATURE_GRID_ICON_CATEGORY_LABELS[category] || category;
        ...
        function renderGrid() {
            ...
            var visibleCount = 0;
            FEATURE_GRID_ICON_LIBRARY.forEach(function (entry) {
                if (activeCategory !== 'all' && s(entry.category || 'general') !== activeCategory) return;
                if (searchTerm && entry._search.indexOf(searchTerm) === -1) return;
                var button = document.createElement('button');
                button.type = 'button';
                button.className = 'metis-menu-icon-option';
                button.setAttribute('data-v2-feature-grid-icon-option', entry.key);
                button.innerHTML = '<span class="metis-menu-icon-option-mark">' + featureGridIconMarkup(entry.key) + '</span><span class="metis-menu-icon-option-label">' + esc(entry.label) + '</span>';
            ...
            clearBtn.setAttribute('data-v2-feature-grid-icon-option', '');
        ...
        panel.style.top = (rect.bottom + 8 + scrollY) + 'px';
        panel.style.left = Math.max(12, rect.left + scrollX) + 'px';
        panel.hidden = false;
        ...
        window.requestAnimationFrame(function () {
            ...
            var top = rect.bottom + 8 + scrollY;
            var maxLeft = scrollX + viewportWidth - panelRect.width - 12;
            if (left > maxLeft) left = Math.max(scrollX + 12, maxLeft);
            if (top + panelRect.height > scrollY + viewportHeight - 12) {
                top = Math.max(scrollY + 12, rect.top + scrollY - panelRect.height - 8);
            }
    ...
    function bootNewsletterEditor() {
        ...
        function openNewsletterTestModal() {
            ...
            if (window.Metis && Metis.ui && Metis.ui.modal) {
                Metis.ui.modal.form('metis-nl-test-modal');
            }
        ...
        function closeNewsletterTestModal() {
            ...
            if (!modal) return;
            if (window.Metis && Metis.ui && Metis.ui.modal) {
                Metis.ui.modal.close('metis-nl-test-modal');
            }
        }
        ...
        function renderShell() {
            ...
            root.innerHTML = '' +
                ...
                '</div>' +
                '</div>' +
                '<div id="metis-nl-test-modal" class="metis-modal-backdrop metis-nl-modal" role="dialog" aria-modal="true" aria-hidden="true" aria-label="Send Test Email">' +
                '<div class="metis-modal metis-nl-modal__dialog">' +
                '<div class="metis-modal-header"><h2 class="metis-modal-title">Send Test Email</h2><button type="button" class="metis-modal-close" id="metis-nl-test-close" data-modal-close="metis-nl-test-modal" aria-label="Close">&times;</button></div>' +
        ...
        function newsletterToolbarDropdown(label, icon, menuHtml, extraClass) {
            return '<div class="metis-se-rich-dropdown ' + esc(extraClass || '') + '">' +
                '<button type="button" class="metis-se-toolbtn metis-se-rich-menu-trigger metis-se-rich-icon-btn" data-rich-toggle="menu" title="' + esc(label) + '" aria-label="' + esc(label) + '" aria-haspopup="menu" aria-expanded="false">' +
                    '<img src="' + esc(iconUrl(icon)) + '" data-icon-fallback="' + esc(iconFallbackUrl(icon)) + '" alt="" aria-hidden="true">' +
                '</button>' +
                '<div class="metis-se-rich-menu" role="menu">' + menuHtml + '</div>' +
            '</div>';
        }
        ...
        function newsletterRichToolbar(targetId) {
            ...
            var blockMenu = [
                ...
                return '<button type="button" role="menuitem" class="metis-se-rich-menu-item" data-rich-action="block" data-rich-target="' + esc(targetId) + '" data-rich-value="' + esc(row[0]) + '">' + esc(row[1]) + '</button>';
            }).join('');
            var sizeMenu = [
                ['default', 'Default'],
                ['sm', 'Small'],
            ...
            var colorMenu = newsletterThemeColorOptions.map(function (row) {
                return '<button type="button" role="menuitem" class="metis-se-rich-menu-item metis-se-rich-menu-item--color" data-rich-action="color" data-rich-target="' + esc(targetId) + '" data-rich-value="' + esc(row[0]) + '">' +
            ...
            var weightMenu = [
                ...
                return '<button type="button" role="menuitem" class="metis-se-rich-menu-item" data-rich-action="weight" data-rich-target="' + esc(targetId) + '" data-rich-value="' + esc(row[0]) + '">' + esc(row[1]) + '</button>';
            }).join('');
            var mergeMenu = [
                ...
                return '<button type="button" role="menuitem" class="metis-se-rich-menu-item" data-rich-action="merge" data-rich-target="' + esc(targetId) + '" data-rich-value="' + esc(row[1]) + '">' + esc(row[0]) + '</button>';
            }).join('');
        ...
        function wire() {
            function richDropdownTrigger(dropdown) {
                return dropdown ? dropdown.querySelector('[data-rich-toggle="menu"]') : null;
            }
            function richDropdownItems(dropdown) {
                return Array.from((dropdown || document).querySelectorAll('.metis-se-rich-menu [role="menuitem"]')).filter(function (node) {
                    return !node.hidden && !node.disabled;
                });
            }
            function closeRichDropdown(dropdown, restoreFocus) {
                if (!dropdown) return;
                dropdown.classList.remove('is-open');
                var trigger = richDropdownTrigger(dropdown);
                if (trigger) {
                    trigger.setAttribute('aria-expanded', 'false');
                    if (restoreFocus && typeof trigger.focus === 'function') trigger.focus();
                }
            }
            function closeAllRichDropdowns(except) {
                root.querySelectorAll('.metis-se-rich-dropdown.is-open').forEach(function (node) {
                    if (node !== except) closeRichDropdown(node, false);
                });
            }
            function openRichDropdown(dropdown, focusPosition) {
                if (!dropdown) return;
                closeAllRichDropdowns(dropdown);
                dropdown.classList.add('is-open');
                var trigger = richDropdownTrigger(dropdown);
                if (trigger) trigger.setAttribute('aria-expanded', 'true');
                if (focusPosition) {
                    var items = richDropdownItems(dropdown);
                    var target = focusPosition === 'last' ? items[items.length - 1] : items[0];
            ...
            root.addEventListener('click', function (e) {
                ...
                if (!richToggle && !richAction && !e.target.closest('.metis-se-rich-dropdown')) {
                    closeAllRichDropdowns();
                }
                if (richToggle) {
                    e.preventDefault();
                    var dropdown = richToggle.closest('.metis-se-rich-dropdown');
                    if (dropdown && dropdown.classList.contains('is-open')) closeRichDropdown(dropdown, false);
                    else openRichDropdown(dropdown);
                    return;
                }
            ...
            root.addEventListener('keydown', function (e) {
                if (e.key === 'Escape') {
                    var blockOverlay = document.getElementById('metis-builder-block-overlay');
                    if (blockOverlay && !blockOverlay.hidden) {
                        e.preventDefault();
                        closeBlockInserter();
                        return;
                    }
                    ...
                    if (previewDrawer && !previewDrawer.hidden) {
                        e.preventDefault();
                        closeBuilderDrawer(previewDrawer);
                        return;
                    }
                    var revisionDrawer = document.getElementById('metis-v2-revision-drawer');
                    if (revisionDrawer && !revisionDrawer.hidden) {
                        e.preventDefault();
                        closeBuilderDrawer(revisionDrawer);
                        return;
                    }
                }
                var richToggle = e.target.closest('button[data-rich-toggle="menu"]');
                if (richToggle) {
                    var toggleDropdown = richToggle.closest('.metis-se-rich-dropdown');
                    if (e.key === 'ArrowDown' || e.key === 'Enter' || e.key === ' ') {
                        e.preventDefault();
                        openRichDropdown(toggleDropdown, 'first');
                        return;
                    }
                    if (e.key === 'ArrowUp') {
                        e.preventDefault();
                        openRichDropdown(toggleDropdown, 'last');
                        return;
                    }
                    if (e.key === 'Escape') {
                        e.preventDefault();
                        closeRichDropdown(toggleDropdown, true);
                    }
                    return;
                }
                var menuItem = e.target.closest('.metis-se-rich-menu [role="menuitem"]');
                if (!menuItem) return;
                var itemDropdown = menuItem.closest('.metis-se-rich-dropdown');
                var items = richDropdownItems(itemDropdown);
                var index = items.indexOf(menuItem);
                if (e.key === 'Escape') {
                    e.preventDefault();
                    closeRichDropdown(itemDropdown, true);
                    return;
                }
                if (e.key === 'ArrowDown') {
                    e.preventDefault();
    ...
    function bootStructuredEditorV2() {
        ...
        var richSelections = sharedRichSelections;
        ...
        function sectionDescription(type) {
            var map = {
                ...
                newsletter_signup: 'Visitor newsletter signup form',
                newsletter_archive: 'Public newsletter archive',
        ...
        function normalizeSection(section, idx) {
            ...
            if (out.type === 'heading') {
                ...
                out.content.media_id = Math.max(0, parseInt(s(content.media_id || '0'), 10) || 0);
                out.content.link_url = s(content.link_url || '');
                ...
                out.content.calendar_id = s(content.calendar_id || '');
                out.content.limit = Math.max(1, Math.min(50, parseInt(s(content.limit || '5'), 10) || 5));
                out.content.view_mode = ['card', 'week', 'calendar'].indexOf(s(content.view_mode || 'card')) === -1 ? 'card' : s(content.view_mode || 'card');
            ...
            } else if (out.type === 'newsletter_signup') {
                out.content.list_ids = normalizeIdList(content.list_ids || []);
                out.content.submit_label = repairMojibakeText(content.submit_label || 'Subscribe') || 'Subscribe';
                out.content.success_message = repairMojibakeText(content.success_message || 'Thanks for subscribing.') || 'Thanks for subscribing.';
            } else if (out.type === 'newsletter_archive') {
                out.content.list_ids = normalizeIdList(content.list_ids || []);
                out.content.limit = Math.max(1, Math.min(50, parseInt(s(content.limit || '12'), 10) || 12));
            }
            return out;
        }
        ...
        function renderVisualBlock(sec, index) {
            ...
            if (type === 'heading') {
                ...
                var signupLabels = normalizeIdList(content.list_ids || []).map(function (id) {
                    return optionLabel(state.options.newsletterLists, id, '');
                }).filter(Boolean);
                body = '<div class="metis-builder-dynamic-card"><strong>Newsletter signup</strong><span>' + esc(signupLabels.length ? signupLabels.join(', ') : 'Default newsletter list') + '</span><small>Button: ' + esc(s(content.submit_label || 'Subscribe')) + '</small></div>';
            } else if (type === 'newsletter_archive') {
                var archiveLabels = normalizeIdList(content.list_ids || []).map(function (id) {
                    return optionLabel(state.options.newsletterLists, id, '');
                }).filter(Boolean);
                body = '<div class="metis-builder-dynamic-card"><strong>Newsletter archive</strong><span>' + esc(archiveLabels.length ? archiveLabels.join(', ') : 'All newsletter lists') + '</span><small>' + esc(String(parseInt(s(content.limit || '12'), 10) || 12)) + ' items</small></div>';
            } else if (type === 'html') {
                ...
                body = '<div class="metis-builder-posts-list"><div><strong>Events</strong><span>' + esc(String(parseInt(s(content.limit || '5'), 10) || 5)) + ' upcoming items • ' + esc(s(content.view_mode || 'card')) + ' view</span></div></div>';
            } else if (type === 'form') {
        ...
        function openBlockInserter(index, trigger) {
            if (!state.canEdit && !(state.id < 1 && state.canCreate)) return;
            ...
            if (overlay) {
                ...
                overlay._metisLastFocus = trigger || document.activeElement;
            }
            if (search) {
        ...
        function closeBlockInserter() {
            ...
            if (overlay) {
                ...
                var lastFocus = overlay._metisLastFocus;
        ...
        function renderSectionContentEditor() {
            ...
            if (sec.type === 'heading') {
                ...
                html += '<input id="metis-v2-image-media-id" type="hidden" value="' + esc(String(Math.max(0, parseInt(s(sec.content.media_id || '0'), 10) || 0))) + '">';
            ...
            if (sec.type === 'columns') {
                ...
                var selectedSignupListIds = normalizeIdList(sec.content.list_ids || []);
                html += '<div class="metis-se-field-row"><label>Newsletter Lists</label>' + categoryChipField('metis-v2-newsletter-signup-list-ids', selectedSignupListIds, 'No newsletter lists available.', state.options.newsletterLists) + '</div>';
                html += '<div class="metis-se-field-row"><label>Submit Label</label><input id="metis-v2-newsletter-signup-submit-label" class="metis-se-input" value="' + esc(s(sec.content.submit_label || 'Subscribe')) + '"></div>';
                html += '<div class="metis-se-field-row"><label>Success Message</label><input id="metis-v2-newsletter-signup-success-message" class="metis-se-input" value="' + esc(s(sec.content.success_message || 'Thanks for subscribing.')) + '"></div>';
            } else if (sec.type === 'newsletter_archive') {
                var selectedArchiveListIds = normalizeIdList(sec.content.list_ids || []);
                html += '<div class="metis-se-field-row"><label>Newsletter Lists</label>' + categoryChipField('metis-v2-newsletter-archive-list-ids', selectedArchiveListIds, 'No newsletter lists available.', state.options.newsletterLists) + '</div>';
                html += '<div class="metis-se-field-row"><label>Item Limit</label><input id="metis-v2-newsletter-archive-limit" class="metis-se-input" type="number" min="1" max="50" value="' + esc(String(parseInt(s(sec.content.limit || '12'), 10) || 12)) + '"></div>';
            }
            host.innerHTML = html;
            bindIconFallbacks(host);
        ...
        function requestEditorConfirmation(options) {
            ...
            if (window.Metis && Metis.ui && Metis.ui.modal) {
                Metis.ui.modal.form('metis-v2-confirm-modal');
            }
            return new Promise(function (resolve) {
                var complete = function (ok) {
                    if (window.Metis && Metis.ui && Metis.ui.modal) {
                        Metis.ui.modal.close('metis-v2-confirm-modal');
                    }
                    modal.onclick = null;
        ...
        function sectionToReusableBlock(section) {
            ...
            if (type === 'heading') data = {
                ...
                src: s(sec.content && sec.content.src || ''),
                media_id: Math.max(0, parseInt(s(sec.content && sec.content.media_id || '0'), 10) || 0),
                link_url: s(sec.content && sec.content.link_url || ''),
                alt: s(sec.content && sec.content.alt || ''),
                caption: s(sec.content && sec.content.caption || ''),
                width: s(sec.content && sec.content.width || ''),
                height: s(sec.content && sec.content.height || '')
            };
        ...
        function closeFeaturedImageModal() {
            ...
            if (!modal) return;
            if (window.Metis && Metis.ui && Metis.ui.modal) {
                Metis.ui.modal.close('metis-v2-featured-image-modal');
            }
        }
        ...
        function openFeaturedImageModal() {
            ...
            if (window.Metis && Metis.ui && Metis.ui.modal) {
                Metis.ui.modal.form('metis-v2-featured-image-modal');
            }
        ...
        function closeInlineImageModal() {
            ...
            if (!modal) return;
            if (window.Metis && Metis.ui && Metis.ui.modal) {
                Metis.ui.modal.close('metis-v2-inline-image-modal');
            }
        ...
        function openInlineImageModal(targetId) {
            ...
            if (window.Metis && Metis.ui && Metis.ui.modal) {
                Metis.ui.modal.form('metis-v2-inline-image-modal');
            }
            if (searchEl) searchEl.focus();
        }
        ...
        function applyUploadedEditorImage(context, media) {
            ...
            if (blockTarget && state.sections[blockTarget.index]) {
                ...
                imageSection.content[blockTarget.field] = mediaPublicUrlForRow(row);
                if (blockTarget.field === 'src') imageSection.content.media_id = Math.max(0, parseInt(s(row.id || '0'), 10) || 0);
            ...
            root.innerHTML = '' +
                '<div class="metis-se-shell metis-builder-shell">' +
                    ...
                    '<div class="metis-se-topbar metis-builder-topbar">' +
                        ...
                        '<div class="metis-se-top-right"><span id="metis-se-save-status" class="metis-se-save-status"></span><button type="button" class="metis-se-nav-btn" data-preview-toggle="1" aria-controls="metis-v2-preview-drawer" aria-expanded="false" aria-haspopup="dialog">Preview</button><button type="button" class="metis-se-nav-btn" data-revisions-toggle="1" aria-controls="metis-v2-revision-drawer" aria-expanded="false" aria-haspopup="dialog">Revisions</button>' + ((state.canEdit || state.canCreate) ? '<button id="metis-v2-publish" type="button" class="metis-se-nav-btn metis-builder-primary-btn">Publish</button>' : '') + '</div>' +
                    '</div>' +
	                    '<div class="metis-se-stage-wrap metis-builder-stage-wrap">' +
	                        ...
	                        '</section>' +
	                    '</div>' +
		                    '<aside id="metis-builder-block-overlay" class="metis-builder-left-panel" hidden aria-hidden="true" role="dialog" aria-modal="true" aria-labelledby="metis-builder-block-overlay-title" tabindex="-1"><div class="metis-builder-drawer-head"><div><strong id="metis-builder-block-overlay-title">Add Block</strong><span id="metis-builder-block-insert-target">Choose where to insert.</span></div><button type="button" class="metis-modal-close" data-block-inserter-close="1" aria-label="Close">&times;</button></div><div class="metis-builder-block-overlay-scroll"><div class="metis-content-panel-title">Blocks</div><div id="metis-v2-block-library" class="metis-content-library-list"></div><div class="metis-content-panel-title">Reusable</div><div id="metis-v2-reusable-blocks" class="metis-content-library-list"></div></div></aside>' +
		                    '<aside id="metis-v2-revision-drawer" class="metis-builder-drawer" hidden aria-hidden="true" role="dialog" aria-modal="true" aria-labelledby="metis-v2-revision-drawer-title" tabindex="-1"><div class="metis-builder-drawer-head"><div><strong id="metis-v2-revision-drawer-title">Revisions</strong><span>Compare or restore a saved version.</span></div><button type="button" class="metis-modal-close" data-drawer-close="revisions" aria-label="Close">&times;</button></div><div id="metis-v2-revision-compare" class="metis-content-revision-compare" hidden></div><div id="metis-v2-revisions" class="metis-content-revisions"></div></aside>' +
	                    '<aside id="metis-v2-preview-drawer" class="metis-builder-drawer metis-builder-preview-drawer" hidden aria-hidden="true" role="dialog" aria-modal="true" aria-labelledby="metis-v2-preview-drawer-title" tabindex="-1"><div class="metis-builder-drawer-head"><div><strong id="metis-v2-preview-drawer-title">Preview</strong><span>Rendered from current blocks.</span></div><button type="button" class="metis-modal-close" data-drawer-close="preview" aria-label="Close">&times;</button></div><div id="metis-v2-preview" class="metis-se-preview"></div></aside>' +
                    '<div id="metis-v2-confirm-modal" class="metis-modal-backdrop metis-v2-confirm-modal" role="dialog" aria-modal="true" aria-hidden="true" aria-labelledby="metis-v2-confirm-title" aria-describedby="metis-v2-confirm-message"><div class="metis-modal metis-v2-confirm-modal__dialog"><div class="metis-modal-header"><h2 id="metis-v2-confirm-title" class="metis-modal-title">Confirm action</h2><button type="button" class="metis-modal-close" data-confirm-cancel="1" data-modal-close="metis-v2-confirm-modal" aria-label="Close">&times;</button></div><div class="metis-modal-body"><p id="metis-v2-confirm-message" class="metis-v2-confirm-modal__message">Continue?</p></div><div class="metis-modal-footer"><button type="button" class="metis-btn" id="metis-v2-confirm-cancel" data-confirm-cancel="1" data-modal-close="metis-v2-confirm-modal">Cancel</button><button type="button" class="metis-btn metis-btn-primary" id="metis-v2-confirm-accept" data-confirm-accept="1">Continue</button></div></div></div>' +
                    (isPostContext() ? '<div id="metis-v2-featured-image-modal" class="metis-modal-backdrop metis-featured-image-modal" role="dialog" aria-modal="true" aria-hidden="true" aria-label="Featured Image Picker"><div class="metis-modal metis-featured-image-modal__dialog"><div class="metis-modal-header"><div><h2 class="metis-modal-title">Choose Featured Image</h2><div id="metis-v2-featured-image-count" class="metis-featured-image-modal__count">Loading images...</div></div><button type="button" class="metis-modal-close" id="metis-v2-featured-image-close" data-modal-close="metis-v2-featured-image-modal" aria-label="Close">&times;</button></div><div class="metis-modal-body"><div class="metis-featured-image-modal__toolbar"><input id="metis-v2-featured-image-search" class="metis-se-input" type="search" placeholder="Search images by name or type"><select id="metis-v2-featured-image-mime" class="metis-se-select"><option value=\"\">All image types</option></select></div>' + editorMediaUploadPanelHtml('featured') + '<div id="metis-v2-featured-image-list" class="metis-media-grid metis-featured-image-modal__grid"></div></div><div class="metis-modal-footer"><button type="button" class="metis-btn" id="metis-v2-featured-image-cancel" data-modal-close="metis-v2-featured-image-modal">Close</button></div></div></div>' : '') +
                    '<div id="metis-v2-inline-image-modal" class="metis-modal-backdrop metis-featured-image-modal" role="dialog" aria-modal="true" aria-hidden="true" aria-label="Inline Image Picker"><div class="metis-modal metis-featured-image-modal__dialog"><div class="metis-modal-header"><div><h2 class="metis-modal-title">Insert Image</h2><div id="metis-v2-inline-image-count" class="metis-featured-image-modal__count">Loading images...</div></div><button type="button" class="metis-modal-close" id="metis-v2-inline-image-close" data-modal-close="metis-v2-inline-image-modal" aria-label="Close">&times;</button></div><div class="metis-modal-body"><div class="metis-featured-image-modal__toolbar"><input id="metis-v2-inline-image-search" class="metis-se-input" type="search" placeholder="Search images by name or type"><select id="metis-v2-inline-image-mime" class="metis-se-select"><option value=\"\">All image types</option></select></div>' + editorMediaUploadPanelHtml('inline') + '<div id="metis-v2-inline-image-list" class="metis-media-grid metis-featured-image-modal__grid"></div></div><div class="metis-modal-footer"><button type="button" class="metis-btn" id="metis-v2-inline-image-cancel" data-modal-close="metis-v2-inline-image-modal">Close</button></div></div></div>' +
                '</div>';
        }
        ...
        function applyOptions(resp) {
            ...
            state.options.newsletterLists = Array.isArray(resp.newsletter_lists) ? resp.newsletter_lists : [];
        ...
        function wire() {
            ...
            function drawerTrigger(drawerId) {
                if (drawerId === 'metis-v2-preview-drawer') return root.querySelector('[data-preview-toggle]');
                if (drawerId === 'metis-v2-revision-drawer') return root.querySelector('[data-revisions-toggle]');
                return null;
            }
            function openBuilderDrawer(drawer, trigger) {
                if (!drawer) return;
                drawer.hidden = false;
                drawer.setAttribute('aria-hidden', 'false');
                drawer._metisLastFocus = trigger || document.activeElement;
                if (trigger) trigger.setAttribute('aria-expanded', 'true');
                var focusTarget = drawer.querySelector('input, select, textarea, button:not([disabled]), [tabindex]:not([tabindex="-1"])');
                window.setTimeout(function () {
                    if (focusTarget && typeof focusTarget.focus === 'function') focusTarget.focus();
                    else if (typeof drawer.focus === 'function') drawer.focus();
                }, 0);
            }
            function closeBuilderDrawer(drawer) {
                if (!drawer) return;
                drawer.hidden = true;
                drawer.setAttribute('aria-hidden', 'true');
                var trigger = drawerTrigger(drawer.id);
                if (trigger) trigger.setAttribute('aria-expanded', 'false');
                var lastFocus = drawer._metisLastFocus;
                drawer._metisLastFocus = null;
                if (lastFocus && document.contains(lastFocus) && typeof lastFocus.focus === 'function') {
                    window.setTimeout(function () { lastFocus.focus(); }, 0);
                }
            }
            ...
            root.addEventListener('click', function (e) {
                var featureGridIconPick = e.target.closest('[data-v2-feature-grid-icon-pick]');
                if (featureGridIconPick) {
                    var featureGridIconField = featureGridIconPick.closest('[data-v2-feature-grid-icon-field]');
                    if (featureGridIconField) {
                        e.preventDefault();
                        e.stopPropagation();
                        if (!featureGridIconPanel || featureGridIconPanel.hidden || featureGridActiveField !== featureGridIconField) {
                            featureGridOpenIconPanel(featureGridIconField, featureGridIconPick);
                        } else {
                ...
                if (previewToggle) {
                    var previewDrawer = document.getElementById('metis-v2-preview-drawer');
                    if (previewDrawer) {
                        openBuilderDrawer(previewDrawer, previewToggle);
                        updatePreview();
                    }
                    return;
                }
                ...
                if (revisionsToggle) {
                    var revisionDrawer = document.getElementById('metis-v2-revision-drawer');
                    if (revisionDrawer) {
                        openBuilderDrawer(revisionDrawer, revisionsToggle);
                        loadRevisionList();
                    }
                    return;
                }
                ...
                if (drawerClose) {
                    ...
                    if (drawer) closeBuilderDrawer(drawer);
                    return;
                }
                ...
                if (insertToggle) {
                    ...
                    openBlockInserter(pickerIndex, insertToggle);
                    return;
                }
                ...
                if (pickInline) {
                    ...
                    if (blockTarget && inlineRow && state.sections[blockTarget.index]) {
                        var imageSection = state.sections[blockTarget.index];
                        imageSection.content = imageSection.content && typeof imageSection.content === 'object' ? imageSection.content : {};
                        imageSection.content[blockTarget.field] = mediaPublicUrlForRow(inlineRow);
                        if (blockTarget.field === 'src') imageSection.content.media_id = Math.max(0, parseInt(s(inlineRow.id || '0'), 10) || 0);
                        if (imageSection.type === 'image' && blockTarget.field === 'src' && !s(imageSection.content.alt || '')) {
                    ...
                    requestInlineImageSize().then(function(sizeChoice) {
                        if (inlineRow && inlineTarget && sizeChoice) {
                            insertHtmlAtSelection(inlineTarget, '<figure class="metis-inline-image is-' + esc(sizeChoice) + '"><img src="' + esc(mediaPublicUrlForRow(inlineRow)) + '" alt="' + esc(s(inlineRow.label || 'Inline image')) + '"></figure>');
                            setDirtyAutosave();
                        }
                        closeInlineImageModal();
                    });
            ...
            root.addEventListener('keydown', function (e) {
                ...
                if (e.key === 'ArrowUp') {
                    e.preventDefault();
                    var prevItem = items[(index - 1 + items.length) % items.length];
                    if (prevItem && typeof prevItem.focus === 'function') prevItem.focus();
                }
            });
            root.addEventListener('input', function (e) {
                var target = e.target;
                ...
                if (target.id === 'metis-v2-newsletter-signup-success-message') { sec.content.success_message = s(target.value || 'Thanks for subscribing.') || 'Thanks for subscribing.'; setDirtyAutosave(); return; }
            ...
            root.addEventListener('change', function (e) {
                ...
                if (target.id === 'metis-v2-newsletter-archive-limit') { sec.content.limit = Math.max(1, Math.min(50, parseInt(s(target.value || '12'), 10) || 12)); renderBuilderCanvas(); setDirtyAutosave(); return; }
